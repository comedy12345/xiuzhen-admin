<template>
    <div class="game-services-container">
        <HeadActions @success="seaech()" v-model:queryParameter="queryParameter"></HeadActions>
        <a-table :dataSource="tableData?.records" :columns="columns" bordered :pagination="pagination" size="small"
            ...
            <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'stat'">
                    {{ record.stat && status[record.stat].text }}
                </template>
            </template>
        </a-table>
...
import { computed, onMounted, reactive, Ref, ref } from "vue-demi";
...
import { IGameServicesForm, IGameServicesList, IQueryParameter } from "@/api/gameServices/gameServicesTypes";
...
// 查询参数
const queryParameter = reactive<IQueryParameter>({
    size: 5,
    current: 1,
    parentTid: 0,
    stat: 1
